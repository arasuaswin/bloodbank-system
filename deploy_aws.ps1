...
Write-Host "`n[0/7] Pre-flight checks..." -ForegroundColor Yellow
...
if (-not (Test-Path "$ROOT\terraform\terraform.tfvars")) {
    Write-Host "  terraform.tfvars not found!" -ForegroundColor Red
    Write-Host "  Creating from example..." -ForegroundColor Yellow
    Copy-Item "$ROOT\terraform\terraform.tfvars.example" "$ROOT\terraform\terraform.tfvars"
    Write-Host ""
    Write-Host "  EDIT this file with your real values, then re-run:" -ForegroundColor Yellow
    Write-Host "    notepad $ROOT\terraform\terraform.tfvars" -ForegroundColor Cyan
    Write-Host ""
    Start-Process notepad "$ROOT\terraform\terraform.tfvars"
    exit 1
}
...
Write-Host "`n[1/7] Provisioning AWS Infrastructure..." -ForegroundColor Yellow
Set-Location -Path "$ROOT\terraform"
terraform init -input=false
terraform apply -auto-approve

# ============ STEP 2: Get Outputs ============
Write-Host "`n[2/7] Reading Terraform Outputs..." -ForegroundColor Yellow
...
Write-Host "`n[3/7] Checking GitHub Connection..." -ForegroundColor Yellow
...
if ($PENDING) {
    ...
    Write-Host ""
    Read-Host "  Press ENTER after you've completed the GitHub connection"
...
Set-Location -Path "$ROOT\bloodbank-next"
docker build -t bloodbank-app .

# ============ STEP 5: Push to ECR ============
Write-Host "`n[5/7] Pushing to Amazon ECR..." -ForegroundColor Yellow
$ECR_HOST = $ECR_REPO_URL.Split('/')[0]
aws ecr get-login-password --region $REGION | docker login --username AWS --password-stdin $ECR_HOST
docker tag bloodbank-app:latest "${ECR_REPO_URL}:latest"
docker push "${ECR_REPO_URL}:latest"

# ============ STEP 6: Run Database Migration ============
Write-Host "`n[6/7] Running Database Migration (Prisma)..." -ForegroundColor Yellow
...
Write-Host "`n[7/7] Deploying to ECS (force new deployment)..." -ForegroundColor Yellow
