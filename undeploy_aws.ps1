...
$raw_output = terraform output -raw rds_endpoint 2>$null
$ErrorActionPreference = $oldErrorAction

$DB_ID = $raw_output -replace '\..*$', ''
if ($DB_ID) {
    try {
        $REGION = 'ap-south-1'
...
Write-Host ''
Write-Host '[2/3] Destroying all AWS resources...' -ForegroundColor Yellow
terraform destroy -auto-approve

# --- Step 3: Cleanup ---
Write-Host ''
Write-Host '[3/3] Cleaning up local state...' -ForegroundColor Yellow
